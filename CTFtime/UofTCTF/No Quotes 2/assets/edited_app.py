...
def login():
    ...
    try:
        ...
        return render_template(
            "login.html",
            error=f"Invalid credentials since error.",
            username=username,
        )
    finally:
        try:
    ...
    if not row:
        return render_template(
            "login.html",
            error="Invalid credentials because of no row.",
            username=username,
        )
    '''if not username == row[0] or not password == row[1]:
        return render_template(
            "login.html",
            error="Invalid credentials since wrong comparison result.",
            username=username,
        )'''
        
    if not username == row[0]:
        return render_template(
            "login.html",
            error="Dif username.",
            username=username,
